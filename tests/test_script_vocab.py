...
class TestScriptVocab(unittest.TestCase):
    ...
    def setUp(self):
        ...
        config = scriptVocabConfig(path='/tmp/file.txt', subs_language='auto', target_language='en', 
                               output='output.txt', input_extension='txt', min_word_size=1, 
                               min_appearance=4, encoding='utf-8')
        self.sv = ScriptVocab(config)
    ...
    def test_create_list_of_files_to_process_with_nonexistent_path(self):
        self.sv.config.path = '/nonexistent/path'
    ...
    @patch('script_vocab.ScriptVocab.ts.translate_text')
    @patch('script_vocab.ScriptVocab.time.sleep')
